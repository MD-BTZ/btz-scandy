...
class MongoDBDatabase:
    ...
    def _connect(self):
        ...
        try:
            ...
            # Versuche ohne Authentifizierung
            try:
                logger.info("Versuche Verbindung ohne Authentifizierung...")
                self._client = MongoClient(
                ...
                self._db = self._client[current_config.MONGODB_DB]
                logger.info(f"MongoDB-Verbindung ohne Authentifizierung hergestellt: {current_config.MONGODB_DB}")
                
            except Exception as e2:
                logger.error(f"MongoDB-Verbindungsfehler auch ohne Authentifizierung: {str(e2)}")
                raise
